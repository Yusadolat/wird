...
- **Connected Library** — import Quran.com bookmarks, inspect User API sync status, and open saved ayat directly into reading or tafsir-grounded reflection
...
Wird integrates **4 Content API groups**, **OAuth 2.0 with PKCE**, and **3 User API groups (7 endpoints)**. Every call has a concrete user-facing purpose, not a token integration.
...
| `GET /bookmarks?type=ayah&key={chapter}` | Locate a remote bookmark before deletion | `removeQuranBookmarkByVerseKey` |
...
| `GET /reading-sessions` | Show recent Quran.com reading-session sync status | `fetchQuranReadingSessions` |
| `POST /activity-days` | Log reading time + verse ranges with IANA timezone for correct day-bucketing | `syncQuranActivityDay` |
| `GET /activity-days` | Show Quran.com activity-day sync status for the last 30 days | `fetchQuranActivityDays` |

Every completed wird emits both `reading-sessions` and `activity-days` records — so the user's progress is visible across any Quran Foundation-connected surface, not just Wird. The Connected Library also reads the user's Quran.com bookmarks and progress status back into Wird, turning OAuth from a sign-in checkbox into a visible cross-app experience.
